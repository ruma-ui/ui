...
function playerReducer(state: PlayerState, action: PlayerAction): PlayerState {
    switch (action.type) {
        ...
        case "SET_PLAYBACK_RATE":
            return { ...state, playbackRate: action.payload, showSettings: false };
        case "SET_ACTIVE_TRACK":
            return { ...state, activeTrack: action.payload, showSettings: false };
        case "SET_VOLUME":
    ...
    const toggleMute = useCallback(
        (e?: React.MouseEvent) => {
            ...
            if (newMuted) {
                ...
                handleVolumeChange(
                    volumeBeforeMute.current > 0.01 ? volumeBeforeMute.current : 0.5,
                );
            }
        },
        [handleVolumeChange],
                                    ...
                                    videoRef.current.textTracks.length > 0 && (
                                        <>
                                            ...
                                            <div className="p-1">
                                                ...
                                                {Array.from(videoRef.current.textTracks).map(
                                                    (track) => (
                                                        <button
                                                            key={track.label}
                                                            onClick={() =>
                                                                handleTrackChange(track.label)
                                                            }
                                                            className="flex w-full items-center justify-between rounded p-1.5 text-left text-xs hover:bg-white/20"
                                                        >
                                                            <span>{track.label}</span>
                                                            {activeTrack === track.label && (
                                                                <IoCheckmark />
                                                            )}
                                                        </button>
                                                    ),
