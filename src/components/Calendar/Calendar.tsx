...
export const Calendar: React.FC<CalendarProps> = ({
    ...
    const start = startOfWeek(startOfMonth(visibleMonth), { weekStartsOn, locale });
        ...
        <div id={id} className={cn(wrapper, className)} role="group" aria-label="Calendar">
                        ...
                        >
                            {Array.from({ length: 12 }).map((_, i) => (
                                <option key={i} value={i}>
                                    {format(new Date(2020, i, 1), "LLLL", { locale })}
                                </option>
                            ))}
                        </select>
            ...
            >
                {days.map((day) => {
                    ...
                    const disabled =
                        outOfRange || predicateDisabled || (!showOutsideDays && outOfMonth);
