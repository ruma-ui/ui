...
import { FaFile, FaFolder, FaGripVertical } from "react-icons/fa";
...
const EventHandlingComponent = (args: Partial<React.ComponentProps<typeof SortableList>>) => {
  const [items, setItems] = useState(sampleItems);
  const [events, setEvents] = useState<string[]>([]);

  const handleReorder = (newItems: typeof sampleItems, fromIndex: number, toIndex: number) => {
    setEvents(prev => [
    ...
    <div className="flex items-start gap-6">
      <div className="flex-1">
        <SortableList
          ...
          items={items}
          onReorder={
            handleReorder as (items: unknown[], fromIndex: number, toIndex: number) => void
          }
...
const WithStateManagementComponent = (args: Partial<React.ComponentProps<typeof SortableList>>) => {
  const [items, setItems] = useState(sampleItems);
...
export const WithStateManagement = {
  render: args => <WithStateManagementComponent {...args} />,
  args: {
    renderItem: item => {
      const typedItem = item as SampleItem;
      return (
